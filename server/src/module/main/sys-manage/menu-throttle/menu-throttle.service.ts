...
export class MenuThrottleService {
  ...
  async selAllMenuThrottle(dto: MenuThrottleSelAllDto): Promise<R> {
    const res = await this.mysqlPrisma.findAll<MenuThrottleDto>('sys_menu_throttle', {
      data: dto,
      orderBy: false,
    });
    return R.ok(res);
  ...
  async insMenuThrottle(dto: MenuThrottleInsOneDto): Promise<R> {
    const res = await this.mysqlPrisma.create<MenuThrottleDto>('sys_menu_throttle', dto);
    await this.cachePermissionService.clearMenuThrottleInCache();
    return R.ok(res);
  }

  async insMenuThrottles(dtos: MenuThrottleInsOneDto[]): Promise<R> {
    const res = await this.mysqlPrisma.createMany<MenuThrottleDto>('sys_menu_throttle', dtos);
    await this.cachePermissionService.clearMenuThrottleInCache();
    return R.ok(res);
  }

  async updMenuThrottle(dto: MenuThrottleUpdOneDto): Promise<R> {
    const res = await this.mysqlPrisma.updateById<MenuThrottleDto>('sys_menu_throttle', dto);
    await this.cachePermissionService.clearMenuThrottleInCache();
    return R.ok(res);
  }

  async updMenuThrottles(dtos: MenuThrottleUpdOneDto[]): Promise<R> {
    const res = await this.mysqlPrisma.updateMany<MenuThrottleDto>('sys_menu_throttle', dtos);
    await this.cachePermissionService.clearMenuThrottleInCache();
    return R.ok(res);
  }

  async delMenuThrottle(ids: number[]): Promise<R> {
    const res = await this.mysqlPrisma.deleteById<MenuThrottleDto>('sys_menu_throttle', ids);
    await this.cachePermissionService.clearMenuThrottleInCache();
    return R.ok(res);
  }
}

...
export class InterfaceGroupController {
  ...
  async selOnesInterfaceGroup(@Query() ids: Record<string, string>): Promise<R> {
    return this.interfaceGroupService.selOnesInterfaceGroup(Object.values(ids).map(Number));
  }

...
const cryptojs = (crypto_ as unknown as { default: typeof crypto_ }).default || crypto_;
...
export const aes = {
  ...
  encrypt: (str: string): string => {
    return cryptojs.AES.encrypt(str, publicConfig.currentConfig().SECRET_KEY).toString();
  },
  /**
   * AES 解密
   * @param encryptedStr
  ...
  decrypt: (encryptedStr: string): string => {
    const bytes = cryptojs.AES.decrypt(encryptedStr, publicConfig.currentConfig().SECRET_KEY);
